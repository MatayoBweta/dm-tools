 ...
 */
public class DatabaseCheckFormproGres extends JPanel implements java.awt.event.ActionListener {

    private static DatabaseCheckFormproGres instance;
    ...
    public static void setChangeDatabase(boolean changeDatabase) {
        DatabaseCheckFormproGres.changeDatabase = changeDatabase;
    }

    /**
     * Creates new form LoginForm
     */
    private DatabaseCheckFormproGres() {
        super();
        try {
            initComponents();
    ...
    public static synchronized DatabaseCheckFormproGres getDefaultForLogin() throws InterruptedException, InvocationTargetException {
        if (instance == null) {
            if (!SwingUtilities.isEventDispatchThread()) {
                SwingUtilities.invokeAndWait(new Runnable() {
                    @Override
                    public void run() {
                        instance = new DatabaseCheckFormproGres();
                    }
                });
            } else {
                instance = new DatabaseCheckFormproGres();
    ...
    public static synchronized DatabaseCheckFormproGres getDefaultForModif() throws InterruptedException, InvocationTargetException {
        if (instance == null) {
            if (!SwingUtilities.isEventDispatchThread()) {
                SwingUtilities.invokeAndWait(new Runnable() {
                    @Override
    ...
    protected void saveValue() throws GeneralSecurityException {
        ...
        node.put("database.user.proGres", Crypto.encrypt(serverUserNameText.getText()));
        node.put("database.password.proGres", Crypto.encrypt(new String(serverPasswordText.getPassword())));
        node.put("database.hostname.proGres", serverHostNameText.getText());
        node.put("database.name.proGres", serverDatabaseNameText.getText());
        node.putInt("database.port.proGres", new Integer(serverPortText.getText()));
    }

    protected void getValue() throws GeneralSecurityException {
        Preferences node = NbPreferences.root();
        serverUserNameText.setText(Crypto.decrypt(node.get("database.user.proGres", Crypto.encrypt("proGresDBUser"))));
        serverPasswordText.setText(Crypto.decrypt(node.get("database.password.proGres", Crypto.encrypt("sqladmin"))));
        serverHostNameText.setText(node.get("database.hostname.proGres", "localhost"));
        serverPortText.setText(new Integer(node.getInt("database.port.proGres", 1433)).toString());
        serverDatabaseNameText.setText(node.get("database.name.proGres", "proGres"));
    }
    ...
    private void initComponents() {
        ...
        serverHostNameLabel.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.serverHostNameLabel.text")); // NOI18N

        serverPortLabel.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.serverPortLabel.text")); // NOI18N

        serverUserNameLabel.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.serverUserNameLabel.text")); // NOI18N

        serverPasswordLabel.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.serverPasswordLabel.text")); // NOI18N

        centralPanel.setLayout(null);

        serverDatabaseNameLabel.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.serverDatabaseNameLabel.text")); // NOI18N
        ...
        entryMessage.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.entryMessage.text_1")); // NOI18N
        ...
        iconLabel.setText(org.openide.util.NbBundle.getMessage(DatabaseCheckFormproGres.class, "DatabaseCheckFormproGres.iconLabel.text_1")); // NOI18N
    ...
    public void actionPerformed(ActionEvent e) {
        switch (e.getActionCommand()) {
            case "Save":
                if (isValidData() && DatabaseUtility.checkProGresDataabse(DatabaseUtility.getMSSQLDatabaseURL(getHostName(), getPort(), getDatabaseName()), getUserID(), new String(serverPasswordText.getPassword()))) {
                    try {
                        saveValue();
                        if (changeDatabase) {
            ...
            case "Test":
                ...
                if (DatabaseUtility.checkProGresDataabse(DatabaseUtility.getMSSQLDatabaseURL(getHostName(), getPort(), getDatabaseName()), getUserID(), new String(serverPasswordText.getPassword()))) {
